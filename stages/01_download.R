...
dataref <- keep(href,~ grepl("*.zip$",.)) |>
  discard(~ grepl(r"((targetscan_70\.zip|targetscan_70_BL_PCT\.zip|TargetScan7_context_scores\.zip)$)", .)) |>
  map_chr(~ paste0("https://www.targetscan.org",.)) # URLs of files to be downloaded
